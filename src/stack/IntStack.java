...
class IntStack {
    // 자료 저장 공간 : 스택용 배열
    private int[] stk;
    // 스택 포인터 : 해당 스택의 자료의 개수와 위치를 파악 가능
    private int ptr;
    // 스택의 용량
    ...
    public IntStack(int maxlength) {
        ...
        } catch (OutOfMemoryError e) { // 생성할 수 없을 때의 예외 처리
            capacity = 0;
        }
    }
    // 실행시 예외 : 스택이 비어있다.
    ...
    public int peek() throws EmptyIntStackException{
        if(ptr <= 0) throw new EmptyIntStackException();
        return stk[ptr-1]; // 스택이 비어 있지 않다면 꼭대기에 있는 stk[ptr-1]의 값을 반환 시킨다.
    }
    ...
    // 스택 안의 모든 데이터를 바닥부터 꼭대기까지 순서대로 출력 기능 : dump()
    public void dump(){
        if(ptr <= 0){
