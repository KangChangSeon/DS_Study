...
public class IntArrayQueue {
    ...
    public int deque() throws EmptyIntArrayQueueException {
        if (this.num <= 0) throw new EmptyIntArrayQueueException();
        int x = que[0];
    ...
    // 큐에서 x를 검색하여 발견하지 못하면 -1을 반환 하는 indexOf() 기능 구현
    public int indexOf(int x){
        for (int i = 0; i < num; i++) {
            if (que[i] == x) return i; // 검색 성공 시 해당 인덱스 전달
        }
        return -1; // 검색 실패
    }
    ...
    // 큐가 비어있는지 확인하는 boolean 타입의 isEmpty()
    public boolean isEmpty(){
        return num <= 0;
    }

    // 큐가 가득 차있는지 확인하는 boolean 타입의 isFull()
    public boolean isFull(){
        return num >= capacity;
    }

    // 큐 안의 모든 데이터를 맨 앞에서 맨 끝 순서로 출력하는 void dump()
    public void dump(){
